from src.calibracao.config_vida import carregar_configuracao_vida
...
def obter_bboxes_vida(frame):
  altura, largura, _ = frame.shape

  config = carregar_configuracao_vida()
...
def extrair_barras(frame):
  bbox_jogador, bbox_inimigo = obter_bboxes_vida(frame)

  x1, y1, x2, y2 = bbox_jogador
  vida_jogador = frame[y1:y2, x1:x2]

  x1, y1, x2, y2 = bbox_inimigo
  vida_inimigo = frame[y1:y2, x1:x2]

  return vida_jogador, vida_inimigo
